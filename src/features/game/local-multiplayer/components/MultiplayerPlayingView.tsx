...
export function MultiplayerPlayingView({
  ...
  return (
    <div className="flex min-h-svh flex-col">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-1.5">
            <span className="rounded px-1.5 py-0.5 text-[10px] font-semibold text-muted-foreground">
              {myTeam === "A" ? teamAName : myTeam === "B" ? teamBName : "—"}
            </span>
            {isHost && (
            ...
            <div
              className={`rounded-full px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wider ${
                isClueMode ? "bg-primary/20 text-primary" : "bg-muted text-muted-foreground"
              }`}
            >
              {isClueMode ? "Clue Mode" : "Observer Mode"}
            </div>
          </div>
        </div>
        <div className="rounded-lg bg-muted/80 px-3 py-1.5 tabular-nums text-lg font-bold text-foreground">
          {timeLeft}s
        </div>
      ...
      {/* Score */}
      <div className="flex shrink-0 justify-center gap-3 px-4 pb-1.5">
        <div
          className={`rounded-xl px-3 py-1.5 ${
            session.current_turn_team === "A" ? "bg-primary/15 text-primary" : "bg-muted text-muted-foreground"
          }`}
        >
          <span className="text-[10px] font-semibold uppercase">{teamAName}</span>
          <span className="ml-1.5 tabular-nums font-bold">{session.score_team_a}</span>
        </div>
        <div
          className={`rounded-xl px-3 py-1.5 ${
            session.current_turn_team === "B" ? "bg-primary/15 text-primary" : "bg-muted text-muted-foreground"
          }`}
        >
          <span className="text-[10px] font-semibold uppercase">{teamBName}</span>
          <span className="ml-1.5 tabular-nums font-bold">{session.score_team_b}</span>
        </div>
      </div>
